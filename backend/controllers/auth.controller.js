...
export async function getMe(req, res) {
  try {
    ...
    await withRLS(user.id, async (t) => {
      // Reload user within RLS transaction to ensure policy compliance
      const secureUser = await User.findByPk(user.id, { transaction: t });
      ...
      res.json({
        ...
        is_vip: !!secureUser.is_vip,
        preferences: secureUser.preferences || {},
        email_verified: secureUser.email_verified
      });
    });
  } catch (err) {
