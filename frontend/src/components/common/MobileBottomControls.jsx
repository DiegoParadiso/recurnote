...
export default function MobileBottomControls({ showSmall, setShowSmall, onToggleLeft, onToggleRight }) {
  return (
    <div className="mobile-controls">
      <button onClick={onToggleLeft} aria-label="Mostrar sidebar izquierdo" className="p-0">
      ...
      <button onClick={() => setShowSmall(prev => !prev)} aria-label="Toggle mostrar pequeño" className="p-0">
        {showSmall ? <EyeOff size={20} /> : <Eye size={20} />}
      </button>

      <button onClick={onToggleRight} aria-label="Mostrar sidebar derecho" className="p-0">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" className="w-5 h-5">
